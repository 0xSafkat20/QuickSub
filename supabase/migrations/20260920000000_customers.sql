...
create table public.quicksub_customers (
...
create table public.quicksub_customer_sessions (
 id text primary key,
 user_id uuid not null references auth.users(id) on delete cascade,
 token text not null,
 expires_at timestamptz not null
);
alter table public.quicksub_orders add column customer_id uuid references auth.users(id) on delete set null;
alter table public.quicksub_orders add column expires_at timestamptz;
create index on public.quicksub_orders(customer_id,created_at desc);
create index on public.quicksub_customer_sessions(expires_at);
...
create function public.quicksub_save_customer(p_user uuid,p_name text,p_contact text,p_reminders boolean)
...
create function public.quicksub_customer_order(p_user uuid,p_id uuid,p_hash text,p_package uuid,p_name text,p_contact text,p_note text,p_expected numeric)
returns jsonb language plpgsql set search_path=public as $$
...
create function public.quicksub_claim_order(p_user uuid,p_id uuid,p_hash text)
...
create function public.quicksub_set_expiry(p_actor uuid,p_id uuid,p_expires timestamptz)
returns void language plpgsql set search_path=public as $$
begin
