...
export default function PasswordRecovery({reset=false}:{reset?:boolean}) {
 ...
 const [tokenHash]=useState(()=>{
  ...
  return new URLSearchParams(url.hash.slice(1)).get('token_hash') || url.searchParams.get('token_hash') || '';
 });
 ...
 setBusy(true);try{
 const result=await api<{message:string}>(reset?'/account/reset-password':'/account/forgot-password',reset?{tokenHash,password:data.get('password'),confirmPassword:data.get('confirmPassword')}:{email:data.get('email')});
