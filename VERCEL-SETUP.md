...
1. Apply supabase/migrations/20260918000000_admin_sessions.sql in Supabase SQL Editor after the previous two migrations.
...
Sessions expire within one hour, are revalidated against Supabase on every request, and are removed on logout. The session table is inaccessible to anon/authenticated roles. Schedule deletion of expired rows periodically with: delete from public.quicksub_admin_sessions where expires_at < now();
