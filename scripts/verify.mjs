...
if(process.argv.includes('--browser'))steps.push(...[['browser-admin','scripts/check-admin.mjs'],['browser-chat','scripts/check-chat.mjs'],['browser-privacy','scripts/check-privacy.mjs'],['browser-storefront','scripts/check-storefront.mjs']].map(([name,path])=>[name,['node',path]]));
