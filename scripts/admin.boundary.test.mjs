...
test('Boundary, authorization and transaction regressions',async t=>{
 ...
 await t.test('failed fulfillment leaves both order and audit unchanged',async()=>{const before=(await env.db.query('select count(*)::int n from quicksub_audit')).rows[0].n;assert.equal((await call('/admin/order/'+payload.id,{status:'delivered',payment_status:'unpaid',delivery_note:'Done'},cookie,'PUT')).status,409);const row=(await env.db.query('select status,payment_status from quicksub_orders where id=$1',[payload.id])).rows[0];assert.deepEqual(row,{status:'pending',payment_status:'unpaid'});assert.equal((await env.db.query('select count(*)::int n from quicksub_audit')).rows[0].n,before);});
 ...
 await t.test('session expiration rejects previously valid cookies',async()=>{clock+=3600001;assert.equal((await call('/admin/session',undefined,cookie)).status,401);});
