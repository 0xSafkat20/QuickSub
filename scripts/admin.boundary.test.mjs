...
test('Boundary, authorization and transaction regressions',async t=>{
 ...
 await t.test('anonymous and ordinary authenticated users cannot read protected tables or execute RPCs',async()=>{for(const role of ['anon','authenticated']){for(const table of ['quicksub_admins','quicksub_products','quicksub_packages','quicksub_orders','quicksub_audit','quicksub_content','quicksub_requests','quicksub_admin_notification_reads']){const r=await env.db.query('select has_table_privilege($1,$2,\'select\') allowed',[role,table]);assert.equal(r.rows[0].allowed,false,role+' '+table);}for(const signature of ['quicksub_create_order(uuid,text,uuid,text,text,text,numeric)','quicksub_admin_write(uuid,text,text,jsonb)','quicksub_overview()','quicksub_customers(integer)','quicksub_mark_admin_notifications_read(uuid,text[],timestamp with time zone)','quicksub_admin_notifications(uuid)'])assert.equal((await env.db.query('select has_function_privilege($1,$2,\'execute\') allowed',[role,signature])).rows[0].allowed,false);}});
